...
public class QuizService {
    ...
    public QuizRes generateQuiz(QuizReq request) {
        try {
    ...
    public EvaluateRes evaluateQuiz(EvaluateReq request) {

        /* Count correct answers - mirrors forEach loop in React */
        int correctCount = 0;
        int totalQuestions = request.getQuestions().size();

        for (EvaluateReq.QuestionAnswerDTO question : request.getQuestions()) {
    ...
    public QuizRes getQuizById(Long id) {
        ...
        try {
            return objectMapper.readValue(quiz.getAiResponse(), QuizRes.class);
        } catch (Exception e) {
